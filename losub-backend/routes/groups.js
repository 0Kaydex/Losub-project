...
// ---------------------------------------------------------------------------
// Messages — two threads per group:
//   'group'   → manager + all members, everyone can read/post
//   'support' → manager <-> Losub admin/owner team, manager-only on this side
// Messages are wiped after 24h (see db.pruneOldMessages).
// ---------------------------------------------------------------------------

function loadMembership(groupId, userId) {
...
// GET /api/groups/:id/messages?thread=group|support
router.get("/:id/messages", (req, res) => {
  db.pruneOldMessages();

  const thread = req.query.thread === "support" ? "support" : "group";
  const { group, membership } = loadMembership(req.params.id, req.userId);

  if (!group) return res.status(404).json({ error: "Group not found." });
  if (!membership) return res.status(403).json({ error: "You're not part of this group." });
  if (thread === "support" && membership.role !== "manager") {
    return res.status(403).json({ error: "Only the group manager can view messages to Losub." });
  }

  const messages = db.prepare(
    "SELECT id, sender_id, sender_role, sender_name, body, created_at FROM messages WHERE group_id = ? AND thread = ? ORDER BY created_at ASC"
  ).all(req.params.id, thread);

  res.json({ messages, expiresAfterHours: 24 });
});

// POST /api/groups/:id/messages  { thread: 'group'|'support', body: string }
router.post("/:id/messages", (req, res) => {
  db.pruneOldMessages();

  const thread = req.body.thread === "support" ? "support" : "group";
  ...
  if (!membership) return res.status(403).json({ error: "You're not part of this group." });
  if (thread === "support" && membership.role !== "manager") {
    return res.status(403).json({ error: "Only the group manager can message Losub." });
  }

  const sender = db.prepare("SELECT fullname FROM users WHERE id = ?").get(req.userId);

  const result = db.prepare(
    "INSERT INTO messages (group_id, thread, sender_id, sender_role, sender_name, body) VALUES (?, ?, ?, ?, ?, ?)"
  ).run(req.params.id, thread, req.userId, membership.role, sender.fullname, body);
  ...
  if (thread === "group") {
    // Notify everyone else in the group (manager included, if a member sent it).
    const others = db.prepare(
      "SELECT user_id FROM group_members WHERE group_id = ? AND user_id != ?"
    ).all(req.params.id, req.userId);
    others.forEach(m => notify(m.user_id, `${sender.fullname}: ${body.slice(0, 80)}`, "group_message", `messages.html?group=${req.params.id}`));
  } else {
    // Notify every admin/owner about a new support message.
    const admins = db.prepare("SELECT id FROM users WHERE role IN ('admin', 'owner')").all();
    admins.forEach(a => notify(a.id, `${sender.fullname} messaged Losub support about a group.`, "group_message", `admin-messaging.html?group=${req.params.id}`));
  }
